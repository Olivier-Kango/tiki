...
if [ ! -d "${BASE}/data/${BRANCH}" ]; then
    ...
    cd ${BASE}/data || exit
    if [ "${BRANCH}" = "trunk" ]; then
        svn checkout --username "${SVNUSER}" "https://svn.code.sf.net/p/tikiwiki/code/${BRANCH}" "${BRANCH}" >> "${LOGFILE}" 2>&1
    else
        svn checkout --username "${SVNUSER}" "https://svn.code.sf.net/p/tikiwiki/code/branches/${VERSION}.x" "${BRANCH}" >> "${LOGFILE}" 2>&1
    fi
...
if [ -d "${PACKDIR}/${BRANCH}" ]; then
    ...
    cd ${TARGET} || exit
    FILES=$(echo ./* | grep -v md5 | grep -v grep)
    for FILE in ${FILES}
    do
