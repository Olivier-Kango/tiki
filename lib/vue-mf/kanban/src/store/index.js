...
export default createStore({
    ...
    actions: {
        initKanban({ commit }, data) {
            commit('setKanban', data)
        },
    ...
    mutations: {
        ...
        addNewColumn(state, data) {
            state.rows.allIds.forEach(rowId => {
                let newId = state.columns.allIds.length ? Math.max(...state.columns.allIds) + 1 : 1
                state.columns.allIds.push(newId)
                state.columns.byId[newId] = { id: newId, title: data.title, cards: [], limit: 10 }
                state.rows.byId[rowId].columns.push(newId)
            })
            let newColId = state.cols.allIds.length ? Math.max(...state.cols.allIds) + 1 : 1;
