...
(function ($) {
    ...
    $.fn.imageAnnotation = function (toCreate, showLinks) {
        ...
        $(".minimize", $editor).on("click", function () {
            endEdit(false);
            return false;
        });

        $(".delete", $editor).on("click", function (event) {
            handleDelete();
            event.preventDefault();
            return false;
        });

        $("#" + containerId + "-editable").on("change", function () {
        ...
        $("form.save-annotations", $container.parent()).on("submit", function () {
