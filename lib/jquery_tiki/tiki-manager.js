...
(function () {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    ...
                    $profile.replaceWith('<input type="text" class="form-control" name="profile" id="profile" placeholder="e.g Personal_Blog_and_Profile" required>');
                }
            },
            error: function() {
                ajaxLoadingHide();
    ...
    if ($vmform.length > 0) {
        ...
        manageRepositoryField($vmform);
        manageApplyFields($vmform);
    }

    if ($form.length > 0) {
        manageRepositoryField($form);
        manageApplyFields($form);
    }
