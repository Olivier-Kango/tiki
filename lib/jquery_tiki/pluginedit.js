...
(function ($) {
    ...
    window.popupPluginForm = function (area_id, type, index, pageName, pluginArgs, isMarkdown, bodyContent, edit_icon, selectedMod, textToReplace) {
            ...
            if (sel && sel.length > 0) {
                ...
                if (sel.length > 0 && sel.substring(0, 1) === '{') { // whole plugin selected
                    ...
                    if (thisType === type.toUpperCase()) { // same plugin
                        ...
                        var stringT = specialCharacterTreatment(sel);
                        sel = stringT[0];
                        var m = sel.match(rx);
                        if (!m) {
        ...
        let paramCompt = 0;
        for (let prop in pluginArgs) {
            pluginArgs[prop] = stringT[1][paramCompt];
            paramCompt++;
        }
    ...
    /*
     * Removed special characters () {} in plugin settings.
     */
    function specialCharacterTreatment(string) {
        let startPosParam = string.indexOf("\"");
        let endPosParam = string.indexOf("\"", startPosParam + 1);
        let param = "";
        let tabParam = new Array();
        let noParam = "";
        let positionSpecCaract = -1;
        let specialCaracts = Array("{", "}", "(", ")");
        let nbSpecialCaractsRemove = 0;
        while (true) {
            if (startPosParam === -1 || endPosParam === -1) {
                break;
            }
            startPosParam++;
            param = string.substring(startPosParam,endPosParam);
            tabParam.push(param);
            specialCaracts.forEach(function(specialCaract) {
                positionSpecCaract = param.indexOf(specialCaract);
                while (positionSpecCaract != - 1) {
                    param = param.substring(0, positionSpecCaract) + param.substring(positionSpecCaract+ 1);
                    nbSpecialCaractsRemove++;
                    positionSpecCaract = param.indexOf(specialCaract);
                }
            });
            string = string.replace(tabParam[tabParam.length - 1], param);
            endPosParam = endPosParam - nbSpecialCaractsRemove;
            noParam = string.substring(endPosParam, string.indexOf("\"", endPosParam + 1));
            if (noParam.indexOf("}") != -1) {
                break;
            }

            startPosParam = string.indexOf("\"", endPosParam + 1);
            endPosParam = string.indexOf("\"", startPosParam + 1);
            nbSpecialCaractsRemove = 0;
        }
        return new Array(string, tabParam);
    }
