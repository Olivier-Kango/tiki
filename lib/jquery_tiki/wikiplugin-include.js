	...
	$('.wikiplugin-include-replace').on('click', function() {
        ...
        let tpl = '<div class="modal-content">' +
                    ...
                    '<div class="modal-footer">' +
                        '<button type="button" class="btn btn-secondary btn-dismiss" data-dismiss="modal">' + tr('Cancel') + '</button>' +
                        '<input onclick="$.fn.replaceInclude(\'' + data.index + '\', \'' + data.page + '\', \'' + data.ticket + '\')" type="button" class="btn btn-primary" value="' + tr('Confirm') + '"/>' +
                    '</div>' +
                '</div>';
