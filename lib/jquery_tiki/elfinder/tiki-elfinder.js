...
function initElFinder(options) {
    ...
    if (options.defaultGalleryId > 0) {
        // seems to be a bug in elFinder as it sets the hash for the dir even if this is false FIXME upsteream...
        location.hash = "";
        // sadly only works on the "normal" file galleries, not user files or attachment gals
        options.startPathHash = 'f1_' + btoa("d_" + options.defaultGalleryId)
