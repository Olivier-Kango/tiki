...
function displayDialog( ignored, list, area_id, clickedElement ) {
    var $modal = $(".footer-modal.show").last(),
        modalTabIndex,
        obj = {
        width: 210,
        bgiframe: true,
...
function dialogFindOpen(area_id) {
    ...
    var s = getTASelection($('#' + area_id)[0]);
    $("#tbFindSearch").val(s).trigger("focus");
}
...
function editorSettings(domId) {
    const $editorSettingsModal = $('.footer-modal:not(.show)').first();
    ...
    $editorSettingsModal.one("shown.bs.modal", event => {
        ...
        $(".btn-primary", $editorSettingsModal).off("click").on("click", function () {
