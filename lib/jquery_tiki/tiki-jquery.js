                ...
                if (relationshipTrackerId) {
                    ...
                    $container.find(':checkbox:checked').each(function (i, cb) {
                        if ($(cb).parent().find('.form-check-label .btn').length == 0) {
                            $(cb).parent().find('.form-check-label').append($container.find('.metadata-icon-template').html());
                        }
                    });
                }
            });
...
$.openModal = function (options) {
    ...
    const $spinner = $("#bootstrap-modal").tikiModal(tr('Loading...'));

    const $modal = $('.modal.fade:not(.show):first')
