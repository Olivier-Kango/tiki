...
$.fn.tiki_popover = function () {
    ...
    list = $container.find('.tips[title!=""], .tikihelp[title!=""]')
        .each(function () {
            ...
            if ($element.attr('title')) {
                $.each(['|', ':', '<br/>', '<br>'], function (key, sep) {
                    ...
                    if (parts.length > 1) {
                        ...
                        $element.data('bs-content', parts.join(sep));
                    }
                });
            } else {
                $element.attr('title', '');
            ...
            if (! $element.data('bs-trigger')) {
                $element.data('bs-trigger', 'hover focus');
            }
            // default Tiki delay
            if (! $element.data('bs-delay')) {
                $element.data('bs-delay', {"show": 250, "hide": 500});
            }
        });

    $.merge(list, $container.find("a[data-bs-toggle=popover]:not(.tips[title!='']):not(.tikihelp[title!=''])"));

    list.filter('.bottom').data('bs-placement', 'bottom');
    list.filter('.left').data('bs-placement', 'left');
    list.filter('.right').data('bs-placement', 'right');
    list.filter('.slow').data('bs-delay', { "show": 1000, "hide": 500 });
    ...
    list.filter('[data-bs-delay^="{"]').each(function () {
        ...
        this.dataset.bsDelay = $(this).data('bs-delay').show;
    ...
    if (modal) {
        ...
        $('.modal-content', target).html(
            '<div class="modal-header">' +
                ...
                '<button type="button" class="close pull-right" data-bs-dismiss="modal">x</button>' +
            '</div>' +
