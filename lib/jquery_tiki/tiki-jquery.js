...
//                        }
        ...
        case "select2":
            ...
            break;
        case "copy":
            return function(copyBtnSelector, getText, onSuccess, onError) {
                if (ClipboardJS.isSupported()) {
                    const clipboard = new ClipboardJS(copyBtnSelector, {
                        text: getText
                    });
                    if (typeof onSuccess === "function") clipboard.on('success', onSuccess);
                    if (typeof onError === "function") clipboard.on('error', onError);
                }
            };
    }    // end switch(func)
};
        ...
        this.each(function () {
            ...
            $(this).load(url, function (response, status) {
                ...
                if (jqueryTiki.useInlineComment && ! jqueryTiki.useInlineAnnotations) {
                    ...
                    $top.noteeditor($('.comment-form', comment_container).last().find('a'), '#note-editor-comment');
                }
...
$(document).ready(function() {
    ...
    if ($(header).hasClass('fixed-top')) {
        $('.middle-wrapper').css('margin-top', $(header).outerHeight() + 5);
    }
});


$(document).tiki('copy')('.icon_copy_code', null, function(element) {
    setTimeout(function()
    {
