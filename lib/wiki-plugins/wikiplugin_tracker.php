        ...
        'params' => [
            ...
            'action_style' => [
                ...
                'description' => tr(
                    ...
                    '<code>action</code>'
                ) . "<code>btn btn-secondary:btn btn-success:btn btn-primary float-end</code>",
                'since' => '14.1',
                'separator' => ':',
        ...
        foreach ($flds['data'] as $i => $f) { // collect additional infos
            if (in_array($f['fieldId'], $outf)) {
                ...
                if (! empty($transactionName) && isset($_SESSION[$transactionName]['values']['ins_' . $f['fieldId']])) {
                    ...
                    // don't add an empty string as the default value for new items so the field handlers can work out if there's a value set of not
        ...
        foreach ($flds['data'] as $f) {
            ...
            if (! in_array($f['fieldId'], $auto_fieldId) && in_array($f['fieldId'], $hidden_fieldId)) {
                ...
                if (! empty($tpl) || ! empty($wiki)) {
                    ...
                    $back .= '<div class="tiki-form-group row tracker_input_label tracker_field' . $f['fieldId'] . '"';
...
FILL;
        ...
        if ($params['formtag'] == 'y') {
            if (empty($wiki) && empty($tpl)) {
                ...
                $back .= '<div class="tiki-form-group row"><div class="input_submit_container btn-bar">';
            }
