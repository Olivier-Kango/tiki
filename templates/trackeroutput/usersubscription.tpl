...
{if $context.list_mode eq 'n'}
    {if !empty($field.maxsubscriptions)}(max : {$field.maxsubscriptions}){/if} :
    {foreach from=$field.users_array name=U_user item=U_user}
        {$U_user.login|userlink}{if !empty($U_user.friends)} (+{$U_user.friends}){/if}{if $smarty.foreach.U_user.last}{else},&nbsp;{$last}{/if}
    {/foreach}
    {if $user}
        <br>
        {if !empty($field.user_subscription)} {tr}You have subscribed{/tr}.{else}{tr}You have not yet subscribed{/tr}.{/if}
        <form method="post" action="{$smarty.server.SCRIPT_NAME}?{query}">
            {if not $field.user_subscription}
                ...
                {/if}
            {else}
                <input type="submit" class="btn btn-primary btn-sm" name="user_unsubscribe" value="{tr}Unsubscribe{/tr}">
            {/if}
        </form>
    {/if}
{/if}
