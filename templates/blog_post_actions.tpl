    ...
    <div class="dropdown-menu">
        ...
        {if $blog_post_context ne 'print'}
            {if ($ownsblog eq 'y') or ($user and $post_info.user eq $user) or $tiki_p_blog_admin eq 'y'}
                    ...
                    <a class="dropdown-item" href="tiki-view_blog.php?blogId={$post_info.blogId}&amp;remove={$post_info.postId}" onclick="confirmPopup('{tr}Delete this item?{/tr}', '{ticket mode=get}')">
            ...
            {if $user and $prefs.feature_notepad eq 'y' and $tiki_p_notepad eq 'y'}
                    {if $blog_post_context eq 'view_blog'}
                        <a class="dropdown-item" href="tiki-view_blog.php?blogId={$post_info.blogId}&amp;savenotepad={$post_info.postId}" onclick="confirmPopup('{tr}Save to notepad?{/tr}', '{ticket mode=get}')">
                            {icon name="notepad"} {tr}Save to notepad{/tr}
                        </a>
                    {else}
                        <a class="dropdown-item" href="tiki-view_blog_post.php?postId={$smarty.request.postId}&amp;savenotepad=1" onclick="confirmPopup('{tr}Save to notepad?{/tr}', '{ticket mode=get}')">
                            {icon name="notepad"} {tr}Save to notepad{/tr}
                        </a>
                    {/if}
            {/if}
