...
DIR_LIST_DEFAULT="${ADMIN_PATH} ${DB_PATH} ${DOC_PATH} ${FILES_PATH} ${IMG_PATH} ${INSTALLER_PATH} ${LANG_PATH} ${LIB_PATH} ${MODULES_PATH} ${PERMISSIONCHECK_PATH} ${STORAGE_PATH} ${TEMP_PATH} ${TEMPLATES_PATH} ${TESTS_PATH} ${THEMES_PATH} ${TIKI_TESTS_TESTS_PATH} ${TIKI_VENDOR_PATH} ${WHELP_PATH}"
DIR_LIST_WRITABLE="${DB_PATH} ${IMG_WIKI_PATH} ${IMG_WIKI_UP_PATH} ${IMG_TRACKERS_PATH} ${STORAGE_PATH} ${STORAGE_PUBLIC_PATH} ${STORAGE_PUBLIC_H5P_PATH} ${STORAGE_FGAL_PATH} ${TEMP_PATH} ${TEMP_CACHE_PATH} ${TEMP_PUBLIC_PATH} ${TEMP_TEMPLATES_C_PATH} ${TEMPLATES_PATH} ${THEMES_PATH} ${WHELP_PATH} ${MODS_PATH} ${FILES_PATH} ${TIKI_TESTS_TESTS_PATH} ${TEMP_UNIFIED_INDEX_PATH} ${TIKI_VENDOR_PATH} ${TIKI_VENDOR_CUSTOM_PATH}"
